...
export function Navbar() {
  ...
  return <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
      <nav className="container mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center gap-8">
          {navLinks.map(link => <li key={link.path}>
              <Link to={link.path} className={cn("text-sm font-medium transition-colors duration-300 hover:text-primary", location.pathname === link.path ? "text-primary" : "text-foreground")}>
                {link.name}
              </Link>
            </li>)}
        </ul>
      ...
      {/* Mobile Navigation */}
      <div className={cn("md:hidden fixed inset-0 top-[65px] bg-background z-40 transition-transform duration-300", isOpen ? "translate-x-0" : "translate-x-full")}>
        <ul className="flex flex-col items-center justify-center h-full gap-8">
          {navLinks.map(link => <li key={link.path}>
              <Link to={link.path} onClick={() => setIsOpen(false)} className={cn("text-2xl font-medium transition-colors duration-300 hover:text-primary", location.pathname === link.path ? "text-primary" : "text-foreground")}>
                {link.name}
              </Link>
            </li>)}
          <li>
            <Link to="/contact" onClick={() => setIsOpen(false)} className="btn-primary text-lg">
              Get in Touch
            </Link>
          </li>
        </ul>
